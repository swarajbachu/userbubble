    ...
    <Container className="border-divide border-x">
      <div className="flex flex-col items-center pt-16">
        ...
        <div className="mt-16 hidden w-full grid-cols-2 divide-x divide-divide border-divide border-t lg:grid">
          ...
          <div className="mask-r-from-90% mask-l-from-90% mask-radial-from-20% relative h-full max-h-[370px] overflow-hidden bg-[radial-gradient(var(--color-dots)_1px,transparent_1px)] [background-size:10px_10px]">
        ...
        <div className="mt-16 flex w-full flex-col divide-y divide-divide overflow-hidden border-divide border-t lg:hidden">
          {tabs.map((tab, index) => (
            ...
            >
              ...
              <p className="relative z-20 mt-2 text-left text-gray-600 text-sm dark:text-neutral-300">
                {tab.description}
              </p>
              <div className="mask-t-from-90% mask-r-from-90% mask-b-from-90% mask-l-from-90% relative mx-auto h-80 w-full overflow-hidden sm:h-80 sm:w-160">
                {tab.skeleton}
              </div>
            </div>
          ))}
...
const Loader = ({ duration = 2500 }: { duration?: number }) => (
  <motion.div
    animate={{ width: "100%" }}
    className="absolute inset-x-0 bottom-0 z-30 h-0.5 w-full rounded-full bg-brand"
    initial={{ width: 0 }}
...
const Canvas = ({
  ...
  <>
    <div className="mask-t-from-50% absolute inset-x-0 z-20 h-full w-full bg-white dark:bg-neutral-900" />
    <PixelatedCanvas
