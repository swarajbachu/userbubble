...
description: "Tailwind CSS is a utility-first CSS framework that allows you to build modern websites without ever leaving your HTML. Lets explore the basics of Tailwind CSS and how it can revolutionize your workflow."
image: "/blog/nextjs.webp"
date: "2021-01-01"
authorName: "Manu Arora"
authorSrc: "/avatars/raj.jpg"
---
...
While the Tailwind approach seems more verbose initially, it eliminates the need to create and maintain separate CSS files, name classes, and establish CSS architecture.
...
3. Generate the Tailwind configuration files:
...
Open your newly created `tailwind.config.js` file and add the paths to your template files:
...
Create a CSS file (e.g., `src/input.css`) with the following Tailwind directives:
...
Now, include the compiled CSS in your HTML:
...
Each class applies a single style property, making it easy to understand what's happening without checking a separate CSS file.
...
To enable this feature, set `darkMode: 'class'` or `darkMode: 'media'` in your `tailwind.config.js`.
...
- Margin/Padding: `m-{size}`, `p-{size}`, `mx-{size}`, `py-{size}`, etc.
...
- Font Weight: `font-thin`, `font-normal`, `font-medium`, `font-bold`, etc.
...
- Background Opacity: `bg-opacity-{value}`
...
- Align Items: `items-start`, `items-center`, `items-end`, etc.
- Grid Template Columns: `grid-cols-{n}`
- Grid Gap: `gap-{size}`

## Customizing Tailwind

Tailwind's power comes from its customizability. You can extend or override the default theme in your `tailwind.config.js`:
...
The `@apply` directive lets you create component classes by combining utilities:
...
- [Tailwind UI](https://tailwindui.com/) - Pre-designed components and templates
